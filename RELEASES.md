...
- **`v1.1.0+docs.3`, same fingerprint:** `evidence/trolley/`, fourteen transcripts of the trolley problem put to leading models cold and with the text loaded, coded for observable actions, with the method and its limits; a "Where things are" table in `README.md` and `AGENTS.md`, keyed to the questions reviewers have asked.
- **`v1.1.0+docs.4`, same fingerprint:** `INTRODUCTION.md` gains "We will believe it when we see it, too," naming what will count as a demonstration; `LICENSE.md` section 1 states what the ban on derivatives guards (standing), that anyone may write their own rules under their own name, and that variation enters by proposal, review, and its own fingerprint, never reaching a wall; `AGENTS.md` carries the author's answer on interpretation and the profile bar's edge cases.
