...
The author's own short statement is `INTRODUCTION.md` (about 2,300 words). Read it if you read one thing.
...
- **Measurement.** Every verdict reads risk scores. A score is as good as its estimator. The text conditions scores on input reliability (`SUBSTRATE.md` section 2) and records that state in the receipt; it does not specify the estimators.
...
- **Thresholds.** Author-set and unvalidated in operation, as above.
- **Governance.** One steward, a no-derivatives license, unsigned succession instruments.
...
| Budget | Read | Words |
|---|---|---|
| Orientation | `README.md`, `INTRODUCTION.md`, `PLAIN-WORDS.md`, `CONSTITUTION.md` | 8,500 |
| The argument in full, and the emergency rule | add `FOREWORD.md`, `UNIVERSAL_PROTOCOL.md`, `START-HERE.md` | + 9,900 |
...
| One principle whole, and the purpose | `CHAPTERS/1.1-Human_Life_Is_Sacred_and_Inviolable.md`; `CHAPTERS/AI_Eudaimonia.md` | 5,000; 7,800 |
| Reference, as needed | `LEXICON.md` 7,700; `CREDENTIALS.md` 5,400; `SUBSTRATE.md` 2,300; `CONSENT.md` 1,600; `CROSS_PRINCIPLE.md` 1,200 | |
...
**The data.** `compiled/bundle.yaml` lists every compiled file with its hash. Start with `compiled/constitution/gate.yaml` (the ten-step order), `compiled/constitution/walls.yaml`, `compiled/protocol/emergency-thresholds.yaml`, `compiled/protocol/receipt-schema.yaml`, `compiled/consent/validity-predicate.yaml`, and `compiled/COVERAGE.yaml`. Skip `compiled/appendices/index.yaml`; it is 124 KB of section hashes and holds no rule. `conformance/examples.json` holds 25 test cases, each tied to the sentence that requires it.
